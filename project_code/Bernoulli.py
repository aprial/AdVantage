...
if __name__ == "__main__":
	...
	BNBmodel = naive_bayes.BernoulliNB(alpha=10.56, binarize=None)
	BNBmodel.fit(X_train, Y_train)
	print BNBmodel.score(X_test, Y_test)
	print metrics.confusion_matrix(Y_test, BNBmodel.predict(X_test))
	# joblib.dump(MNBmodel, 'GNB_ft0.pkl')
	##--linear_regression
	# # print Y_train
	...
	##--
	print '0 fine tune'
	classif_BNB = naive_bayes.BernoulliNB(binarize=None)
	# classif_SVM_lr.fit(X_test, Y_test)
	parameters = {'alpha':np.logspace(-3., 3.)}
	clf = grid_search.GridSearchCV(classif_BNB, parameters, cv = 5)
	...
	joblib.dump(clf, 'BNB_ft0.pkl')
	a_old = None
	counter = 0
	while a_old != clf.best_params_['alpha'] and counter < 10:
		a_old = clf.best_params_['alpha']
		clf = joblib.load('BNB_ft%s.pkl'%counter)
		counter += 1
		print np.linspace(clf.best_params_['alpha']*.9, clf.best_params_['alpha']*1.1, num = 51)
		parameters = {'alpha':np.linspace(clf.best_params_['alpha']*.9, clf.best_params_['alpha']*1.1, num=51)}
		classif_BNB = naive_bayes.BernoulliNB(binarize=None)
		clf = grid_search.GridSearchCV(classif_BNB, parameters, cv = 5)
		clf.fit(X_train, Y_train)
		print clf.best_estimator_
		print clf.best_score_
		print clf.best_params_
		joblib.dump(clf, 'BNB_ft%s.pkl'%counter)
		conf_matrix = metrics.confusion_matrix(Y_test, clf.predict(X_test))
		print conf_matrix
		print clf.score(X_test, Y_test)
